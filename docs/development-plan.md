...
| Vendor settlement | Post-approval settlement (`settlement_status`: `kyc_pending → kyc_submitted → agreement_ready → payment_requested → paid`). After approval the vendor submits **owner (seller) details + the owner's payout bank account + KYC documents** (RC, PAN, Aadhaar, NOC, key photo, owner-with-vehicle photo, cancelled cheque, plus optional extras); staff **verify the documents** (approve or send back). Payment is made to the **owner's** account (shown on the agreement). Verification unlocks a **dynamic pre-filled agreement PDF** (with the real owner/seller name + registration number, Form 29 & 30). The vendor then **requests payment** and staff **record the payment** (amount, mode, reference, date + proof screenshot) → *Paid*. Owner details & documents live on the submission only — **never copied into the purchase lead**. Notifications at each hand-off. | **Done** |
