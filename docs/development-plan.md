...
|---|---|---|
| Purchase by Vendor | Self-service **vendor partner portal** (`/vendor`): register → admin activates → submit vehicles with vehicle details (**registration number required**), gallery + damaged-part images (multi-file upload), a Pass/Fail/NA condition checklist with per-item ratings and an **auto-computed, read-only overall rating**, and an expected amount. Staff review queue (`/admin/vendor-submissions`) + partner activation (`/admin/vendor-partners`); **approval creates a purchase lead** (`source = vendor`) that enters the existing purchase pipeline. Vendor Partner role, scoped policies, multi-channel notifications on register/submit/review. | **Done** |
| Per-employee custom permissions | RBAC is role-based, but the **Employee** editor now also has a **Custom Permissions** panel: an admin (Super Admin, or any role holder with `roles.update`) can grant individual actions to one employee **on top of their roles**, using Spatie direct permissions. Actions already covered by the employee's roles show ticked-and-locked (`· role`); non-Super-Admins can only grant permissions they hold themselves (no privilege escalation). `can()` merges role + direct grants automatically. | **Done** |
| Vendor settlement | Post-approval settlement (`settlement_status`: `kyc_pending → kyc_submitted → agreement_ready → payment_requested → paid → stocked`). After approval the vendor submits **owner (seller) details, chassis number, hypothecation status, the owner's payout bank account, and KYC documents** — **RC & Aadhaar as front + back**, PAN, chassis-plate photo, owner-with-vehicle photo, cancelled cheque, and **NOC + Form 35 required only under hypothecation** (plus optional insurance/PUC/tax/fitness/permit/service-history/invoice). Staff **verify each document in a table** (status pending → verified/rejected/N-A, with attachment + number/validity/remarks, mirroring the purchase-lead verification screen), and **each row records which employee verified it and when**; the **agreement is issued only when every required document is verified** (or the set is sent back for correction). Document verification is its own RBAC right — **`vendor-submissions.verify-documents`** — that the admin can grant to any role (a ready-made **Document Verifier** role ships with it), separate from full review/approve authority. Keys availability (Both/One/None) is recorded in the condition report. Payment is made to the **owner's** account (shown on the agreement). Verification unlocks a **dynamic pre-filled agreement PDF** (with the real owner/seller name + registration number, Form 29 & 30). The vendor then **requests payment** and staff **record the payment** (amount, mode, reference, date + proof screenshot) → *Paid*. Finally staff **confirm possession** (document/key checklist + odometer + fuel) which **auto-creates the inventory (stock) entry** and links it to the submission → *Stocked*. Owner details & documents live on the submission only — **never copied into the purchase lead**. Notifications at each hand-off. | **Done** |
