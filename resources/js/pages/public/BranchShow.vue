...
const props = defineProps<{
    branch: { id: number; name: string; slug: string; city: string | null; state: string | null; address: string | null; phone: string | null; email: string | null; latitude: string | null; longitude: string | null };
...
<template>
    <SeoHead :title="`${branch.name} — Car4Sales`" :description="`Cars available at Car4Sales ${branch.name}, ${branch.city ?? ''}. Visit for a test drive.`" :json-ld="jsonLd" />

    <PublicLayout>
        <div class="mx-auto max-w-7xl px-4 py-8">
            <nav class="mb-4 flex items-center gap-1 text-xs text-neutral-500">
                <Link href="/branches" class="hover:text-brand-maroon">Branches</Link><span>/</span><span class="text-neutral-800">{{ branch.name }}</span>
            </nav>
            ...
            <div class="rounded-2xl border bg-white p-6 shadow-sm">
                ...
                <div class="mt-2 flex flex-wrap gap-4 text-sm text-neutral-600">
                    <span class="flex items-center gap-1"><MapPin class="size-4 text-brand-orange" /> {{ branch.address ?? [branch.city, branch.state].filter(Boolean).join(', ') }}</span>
