...
async function requestOtp() {
    ...
    if (!/^[0-9]{10}$/.test(form.mobile)) {
        otpError.value = 'Enter a valid 10-digit mobile number.';
        return;
    }
    otpBusy.value = true;
...
<template>
    <SeoHead
        title="Sell Your Car — Car4Sales"
        description="Sell your car at the best price. Free inspection, instant quote and hassle-free paperwork."
    />
        ...
        <section class="bg-gradient-to-br from-brand-yellow to-brand-gold py-12 text-brand-maroon">
            <div class="mx-auto max-w-4xl px-4 text-center">
                ...
                <p class="mx-auto mt-2 max-w-xl">
                    Get the best price with a free inspection and instant quote. Fill in your details and our team will call you.
                </p>
            </div>
        </section>
        ...
        <div class="mx-auto max-w-3xl px-4 py-10">
            <form class="grid gap-4 rounded-2xl border bg-white p-6 shadow-sm" @submit.prevent="submit">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <input v-model="form.seller_name" placeholder="Full name *" required class="w-full rounded-lg border px-3 py-2.5 text-sm" />
                        <p v-if="form.errors.seller_name" class="mt-1 text-xs text-brand-red">{{ form.errors.seller_name }}</p>
                    </div>
                    <div>
                        <input
                            v-model="form.mobile"
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    />
                    <input
                        v-model.number="form.expected_price"
                        type="number"
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <input
                        v-model="form.preferred_inspection_location"
                ...
                <p v-else-if="otpRequired && otpStage === 'verified'" class="flex items-center gap-1 text-xs font-medium text-green-600">
                    <CheckCircle2 class="size-4" /> Mobile verified
                </p>
                ...
                <button
                    ...
                    class="rounded-lg bg-brand-maroon px-5 py-3 text-sm font-bold text-white disabled:opacity-50"
                >
                    Get My Quote
                </button>
            </form>
