...
const form = useForm({
    ...
    phone: '',
    email: '',
    base_interest_rate: null as number | null,
...
function confirmDelete() {
    ...
    deleting.value = true;
    router.delete(`/admin/lenders/${deleteTarget.value.id}`, {
        preserveScroll: true,
        onFinish: () => {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            ...
            <div class="overflow-x-auto rounded-xl border border-sidebar-border/70 dark:border-sidebar-border">
                <table class="w-full min-w-[720px] text-sm">
                    ...
                    <tbody>
                        <tr v-if="lenders.data.length === 0">
                            <td colspan="6" class="px-4 py-10 text-center text-muted-foreground">No lenders yet.</td>
                        </tr>
                        <tr v-for="l in lenders.data" :key="l.id" class="border-b last:border-0 hover:bg-muted/30">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-end gap-1">
                                    <Button v-if="can.update" variant="ghost" size="icon" aria-label="Edit lender" @click="openEdit(l)"
                                        ><Pencil class="size-4"
                                    /></Button>
                                    <Button v-if="can.delete" variant="ghost" size="icon" aria-label="Delete lender" @click="deleteTarget = l"
        ...
        <Dialog v-model:open="dialogOpen">
            <DialogContent class="sm:max-w-md">
                <DialogHeader
                    ><DialogTitle>{{ editing ? 'Edit Lender' : 'New Lender' }}</DialogTitle></DialogHeader
                >
                <form class="grid gap-4" @submit.prevent="submit">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="grid gap-2">
                            <Label for="l-contact">Contact Person</Label><Input id="l-contact" v-model="form.contact_person" />
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="grid gap-2">
                            <Label for="l-rate">Base Rate (%)</Label
                            ><Input id="l-rate" v-model.number="form.base_interest_rate" type="number" step="0.01" />
                        </div>
                    </div>
