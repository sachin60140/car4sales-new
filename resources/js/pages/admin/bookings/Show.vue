...
const props = defineProps<{
    ...
    ledger: {
        outstanding: number;
        entries: { id: number; type: string; head: string; amount: string; is_reversal: boolean; remarks: string | null; posted_at: string }[];
    } | null;
                ...
                <div>
                    <div class="flex items-center gap-3">
                        ...
                        <span
                            ...
                            :class="statusStyle[booking.status] ?? 'bg-brand-yellow/20 text-brand-maroon dark:text-brand-yellow'"
                        >
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">
                        ...
                        <Link v-if="booking.vehicle" :href="`/admin/inventory/${booking.vehicle.id}`" class="underline">{{
                            booking.vehicle.stock_number
                        }}</Link>
            ...
            <div class="grid grid-cols-2 gap-3 md:grid-cols-4">
                <Card
                    ><CardContent class="p-4"
                        ...
                        <p class="text-lg font-bold">{{ money(booking.selling_price) }}</p></CardContent
                    ></Card
                >
                <Card
                    ><CardContent class="p-4"
                        ><p class="text-xs text-muted-foreground">Discount</p>
                        <p class="text-lg font-bold">{{ money(booking.discount_amount) }}</p></CardContent
                    ></Card
                >
                <Card
                    ><CardContent class="p-4"
                        ><p class="text-xs text-muted-foreground">Net Payable</p>
                        <p class="text-lg font-bold text-brand-maroon dark:text-brand-yellow">{{ money(netPayable) }}</p></CardContent
                    ></Card
                >
                <Card
                    ><CardContent class="p-4"
                        ><p class="text-xs text-muted-foreground">Paid</p>
                        <p class="text-lg font-bold text-emerald-600">{{ money(paidAmount) }}</p></CardContent
                    ></Card
                >
            </div>
                ...
                <div class="space-y-4 lg:col-span-2">
                    ...
                    <Card v-if="booking.status === 'approval_pending' && booking.approval_request">
                        <CardContent class="flex items-center gap-2 py-4 text-sm">
                            <span class="rounded-full bg-brand-orange/15 px-2 py-0.5 text-xs font-medium text-brand-orange"
                                >Awaiting discount approval</span
                            >
                    ...
                    <Card>
                        ...
                        <CardContent class="grid gap-3">
                            <div
                                v-if="can.update && !['cancelled', 'refunded', 'forfeited'].includes(booking.status)"
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody>
                                    <tr v-if="!booking.payments?.length">
                                        <td colspan="5" class="py-4 text-center text-muted-foreground">No payments.</td>
                                    </tr>
                                    <tr v-for="p in booking.payments" :key="p.id" class="border-b last:border-0">
                                        ...
                                        <td class="py-2 text-right">
                                            <button
                                                v-if="can.reversePayment && p.status === 'received' && p.type !== 'refund'"
                                                class="text-xs text-brand-red underline"
                    ...
                    <Card v-if="ledger">
                        <CardHeader class="flex flex-row items-center justify-between">
                            ...
                            <span class="text-sm"
                                >Outstanding:
                                <strong :class="ledger.outstanding > 0 ? 'text-brand-red' : 'text-emerald-600'">{{
                        ...
                        <CardContent>
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b text-left text-muted-foreground">
                                        <th class="py-2 pr-3 font-medium">Head</th>
                                ...
                                <tbody>
                                    <tr
                                        ...
                                        :class="e.is_reversal ? 'italic text-muted-foreground' : ''"
                                    >
                                        <td class="py-2 pr-3 capitalize">
                                            {{ (e.head ?? '').replace(/_/g, ' ') }}<span v-if="e.is_reversal"> (reversal)</span>
                                        </td>
                            ...
                            <p class="mt-2 text-xs text-muted-foreground">
                                Ledger entries are append-only; corrections post a reversal, never a delete.
                            </p>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <ol class="space-y-2 text-sm">
                                <li
                                    v-for="h in booking.status_histories"
                                    :key="h.id"
                ...
                <div class="space-y-4">
                    <Card>
                        ...
                        <CardContent class="grid grid-cols-2 gap-y-2 text-sm">
                            ...
                            <span
                                ><Link v-if="booking.lead" :href="`/admin/sales-leads/${booking.lead.id}`" class="underline">{{
                    ...
                    <Card v-if="booking.payment_mode === 'finance'">
                        ...
                        <CardContent class="grid gap-2 text-sm">
                            <div v-if="financeApplication" class="flex items-center justify-between">
                                <Link :href="`/admin/finance/${financeApplication.id}`" class="font-mono text-xs underline">{{
                                    financeApplication.application_number
                                }}</Link>
                                <span class="rounded-full bg-muted px-2 py-0.5 text-xs capitalize">{{
                                    (financeApplication.status ?? '').replace(/_/g, ' ')
                                }}</span>
                            </div>
                    ...
                    <Card>
                        ...
                        <CardContent class="grid gap-2 text-sm">
                            <div v-if="invoice" class="flex items-center justify-between">
                                ...
                                <a v-if="invoice.download" :href="invoice.download" class="text-xs text-brand-maroon underline dark:text-brand-yellow"
                                    >Download PDF</a
                                >
                            </div>
                            <Button
                                v-else-if="
                                    can.invoice &&
                                    ['confirmed', 'payment_pending', 'finance_pending', 'ready_for_delivery', 'delivered'].includes(booking.status)
                                "
                                size="sm"
                                variant="outline"
                                @click="post(`/admin/bookings/${booking.id}/invoice`)"
                        ...
                        <CardContent class="grid gap-2">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div class="grid gap-1">
                                    <Label class="text-xs">Refund ₹</Label
                                    ><Input v-model.number="cancelForm.refund_amount" type="number" class="h-9" />
                                </div>
                                <div class="grid gap-1">
                                    <Label class="text-xs">Forfeit ₹</Label
                                    ><Input v-model.number="cancelForm.forfeit_amount" type="number" class="h-9" />
                                </div>
                            </div>
                            <Button size="sm" variant="destructive" :disabled="!cancelForm.reason || cancelForm.processing" @click="requestCancel"
                    ...
                    <Card v-if="pendingCancellation && can.approveCancel">
                        ...
                        <CardContent class="grid gap-2 text-sm">
                            ...
                            <Button size="sm" @click="post(`/admin/booking-cancellations/${pendingCancellation.id}/approve`)"
                                >Approve &amp; Release Vehicle</Button
                            >
                        </CardContent>
                    </Card>
                    ...
                    <Card
                        v-if="
                            ...
                            Number(approvedCancellation.refund_amount) > 0 &&
                    ...
                    >
                        <CardContent class="py-4">
                            <Button size="sm" @click="post(`/admin/booking-cancellations/${approvedCancellation.id}/refund`)"
                                >Raise Refund for Approval ({{ money(approvedCancellation.refund_amount) }})</Button
                            >
                        </CardContent>
                    </Card>
                    ...
                    <Card v-if="pendingRefund && can.refund">
                        ...
                        <CardContent class="grid gap-2">
                            ...
                            <Button size="sm" @click="post(`/admin/refunds/${pendingRefund.id}/pay`, { method: 'bank_transfer' })"
                                >Mark Refund Paid</Button
                            >
                        </CardContent>
                    </Card>
                </div>
