...
const followupOutcomes = ['call_later', 'interested', 'visit_scheduled', 'test_drive_scheduled', 'finance_required', 'exchange_required', 'booking_expected', 'busy', 'no_answer', 'switched_off'];
...
// Create booking
const bookingForm = useForm({ vehicle_id: null as number | null, selling_price: null as number | null, booking_amount: 0, discount_amount: 0, payment_mode: 'cash' });
function createBooking() {
    bookingForm
        .transform((d) => ({ ...d, sales_lead_id: lead.value.id }))
        .post('/admin/bookings', { preserveScroll: false });
}
            ...
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <div class="flex items-center gap-3">
                        ...
                        <span class="rounded-full px-2.5 py-0.5 text-xs font-medium capitalize" :class="statusStyle[lead.status] ?? 'bg-brand-yellow/20 text-brand-maroon dark:text-brand-yellow'">
                        ...
                        <CardContent class="grid gap-3">
                            <div class="grid gap-3 sm:grid-cols-2">
                                <div class="grid gap-1">
                                    ...
                                    <select v-model="callForm.outcome" class="h-9 rounded-md border border-input bg-transparent px-3 text-sm shadow-sm">
                                        <option v-for="o in callOutcomes" :key="o.value" :value="o.value">{{ o.label }}</option>
                                    </select>
                                </div>
                                <div class="grid gap-1">
                                    <Label class="text-xs">Channel</Label>
                                    <select v-model="callForm.channel" class="h-9 rounded-md border border-input bg-transparent px-3 text-sm shadow-sm">
                                        <option value="call">Call</option><option value="whatsapp">WhatsApp</option><option value="sms">SMS</option><option value="email">Email</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div v-if="needsReason" class="grid gap-1">
                                ...
                                <select v-model="callForm.lost_reason_id" class="h-9 rounded-md border border-input bg-transparent px-3 text-sm shadow-sm">
                            ...
                            <textarea v-model="callForm.remarks" rows="2" placeholder="Remarks" class="rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm" />
                    ...
                    <Card v-if="can.scheduleVisit || can.scheduleTestDrive || can.createBooking">
                        ...
                        <CardContent class="grid gap-4">
                            ...
                            <div v-if="can.scheduleVisit" class="flex flex-wrap items-end gap-2 border-b pb-3">
                                ...
                                <Button size="sm" variant="outline" :disabled="!visitForm.scheduled_at || visitForm.processing" @click="scheduleVisit">Book Visit</Button>
                            </div>
                            ...
                            <div v-if="can.scheduleTestDrive" class="border-b pb-3">
                                ...
                                <div v-else class="grid gap-2">
                                    <select v-model="tdForm.vehicle_id" class="h-9 rounded-md border border-input bg-transparent px-3 text-sm shadow-sm">
                                        <option :value="null">Select vehicle…</option>
                                        <option v-for="v in availableVehicles" :key="v.id" :value="v.id">{{ v.stock_number }} — {{ v.make }} {{ v.model }}</option>
                                    </select>
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        <Button size="sm" :disabled="!tdForm.vehicle_id || !tdForm.scheduled_at || tdForm.processing" @click="scheduleTestDrive">Schedule</Button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-if="can.createBooking">
                                ...
                                <div v-else class="grid gap-2">
                                    <select v-model="bookingForm.vehicle_id" class="h-9 rounded-md border border-input bg-transparent px-3 text-sm shadow-sm">
                                        <option :value="null">Select vehicle…</option>
                                        <option v-for="v in availableVehicles" :key="v.id" :value="v.id">{{ v.stock_number }} — {{ v.make }} {{ v.model }} ({{ money(v.asking_price) }})</option>
                                    </select>
                                    <div class="grid grid-cols-2 gap-2">
                                        ...
                                        <select v-model="bookingForm.payment_mode" class="h-9 rounded-md border border-input bg-transparent px-3 text-sm shadow-sm">
                                            <option value="cash">Cash</option><option value="finance">Finance</option>
                                        </select>
                                    </div>
                                    <div class="flex gap-2">
                                        <Button size="sm" :disabled="!bookingForm.vehicle_id || !bookingForm.selling_price || bookingForm.processing" @click="createBooking">Create Booking</Button>
                            ...
                            <div v-if="lead.bookings?.length || lead.test_drives?.length || lead.visits?.length" class="grid gap-1 border-t pt-3 text-sm">
                                <Link v-for="b in lead.bookings" :key="'b' + b.id" :href="`/admin/bookings/${b.id}`" class="flex items-center justify-between hover:underline">
                                    <span class="font-mono text-xs">{{ b.booking_number }}</span>
                                    <span class="rounded-full bg-muted px-2 py-0.5 text-xs capitalize">{{ (b.status ?? '').replace(/_/g, ' ') }}</span>
                                </Link>
                                <div v-for="t in lead.test_drives" :key="'t' + t.id" class="flex items-center justify-between text-muted-foreground">
                                    <span class="font-mono text-xs">{{ t.td_number }}</span><span class="text-xs capitalize">test drive · {{ t.status }}</span>
                                </div>
                                <div v-for="vi in lead.visits" :key="'v' + vi.id" class="flex items-center justify-between text-muted-foreground">
                                    <span class="font-mono text-xs">{{ vi.visit_number }}</span><span class="text-xs capitalize">visit · {{ (vi.status ?? '').replace(/_/g, ' ') }}</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent>
                            <ol class="space-y-3">
                                <li v-for="a in lead.activities" :key="a.id" class="flex gap-3 border-b pb-3 text-sm last:border-0 last:pb-0">
                                    <span class="mt-1 size-2 shrink-0 rounded-full" :class="a.type === 'call' ? 'bg-brand-orange' : a.type === 'status' ? 'bg-brand-maroon dark:bg-brand-yellow' : 'bg-muted-foreground'" />
                                    <div class="flex-1">
                                        <p>{{ a.summary }}</p>
                                        <p class="text-xs text-muted-foreground">{{ a.user?.name ?? 'System' }} · {{ new Date(a.created_at).toLocaleString() }}</p>
                                    </div>
                ...
                <div class="space-y-4">
                    <Card v-if="can.update && allowedTransitions.length">
                        ...
                        <CardContent class="grid gap-2">
                            ...
                            <select v-if="transitionNeedsReason" v-model="transitionForm.lost_reason_id" class="h-9 rounded-md border border-input bg-transparent px-3 text-sm shadow-sm">
                                <option :value="null">Lost reason…</option>
                                <option v-for="r in lostReasons" :key="r.id" :value="r.id">{{ r.label }}</option>
                            </select>
                            <p v-if="transitionForm.errors.lost_reason_id" class="text-xs text-brand-red">{{ transitionForm.errors.lost_reason_id }}</p>
                    ...
                    <Card v-if="can.assign">
                        ...
                        <CardContent class="grid gap-2 text-sm">
                            <p class="text-muted-foreground">Telecaller: <strong class="text-foreground">{{ lead.telecaller?.name ?? 'Unassigned' }}</strong></p>
                            <p class="text-muted-foreground">Sales Exec: <strong class="text-foreground">{{ lead.sales_executive?.name ?? 'Unassigned' }}</strong></p>
                            <div class="mt-1 grid gap-2 border-t pt-2">
                                <select v-model="assignForm.role" class="h-9 rounded-md border border-input bg-transparent px-3 text-sm shadow-sm">
                                    <option value="telecaller">Telecaller</option><option value="sales_executive">Sales Executive</option>
                                </select>
                                <select v-model="assignForm.user_id" class="h-9 rounded-md border border-input bg-transparent px-3 text-sm shadow-sm">
                                    <option :value="null">Unassign</option>
                                    <option v-for="u in (assignForm.role === 'telecaller' ? telecallers : salesExecutives)" :key="u.id" :value="u.id">{{ u.name }}</option>
                                </select>
                    ...
                    <Card>
                        ...
                        <CardContent class="grid grid-cols-2 gap-y-2 text-sm">
                            <span class="text-muted-foreground">Budget</span><span>{{ lead.budget_min || lead.budget_max ? `₹${Number(lead.budget_min ?? 0).toLocaleString('en-IN')} – ₹${Number(lead.budget_max ?? 0).toLocaleString('en-IN')}` : '—' }}</span>
                            ...
                            <span>
                                <Link v-if="lead.interested_vehicle" :href="`/admin/inventory/${lead.interested_vehicle.id}`" class="underline">{{ lead.interested_vehicle.stock_number }}</Link>
                                <span v-else>—</span>
                            </span>
